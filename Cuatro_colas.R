...
# Se generan valores con distribución normal (se toman los valores absolutos para que sean positivos)
...
# Probabilidades para cada caja:
# - Caja 1: 20%
# - Caja 2: 30%
# - Caja 3: 25% (con tiempos de trámite Poisson, media 4 min)
# - Caja 4: 25% (nueva, con tiempos de trámite uniformes entre 6 y 12 min)
cajas <- sample(c(1, 2, 3, 4), n_clientes, replace = TRUE, prob = c(0.2, 0.3, 0.25, 0.25))
...
# - Caja 3: Distribución Poisson con media de 4 minutos.
# - Caja 4: Distribución uniforme entre 6 y 12 minutos.
...
for (i in 1:n_clientes) {
  if (cajas[i] == 1) {
    ...
    tiempos_tramite[i] <- runif(1, min = 6, max = 12)
  }
}
